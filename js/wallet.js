// FOMO Chicken代币配置信息
const tokenInfo = {
    contractAddress: '0x98925bc379b33b3a2e752c223b77711be4df4444',
    decimals: 18,
    symbol: '$FC',
...
const recipientAddress = '0xceaa4677eeb97a2e628da9c6de8567268f0ebb82';
...
安装完成后，请刷新页面重试。
    ...
    async updateBalance() {
        try {
            ...
            if (balanceElement) {
                balanceElement.textContent = `${displayBalance} $FC`;
            ...
            if (modalBalanceElement) {
                modalBalanceElement.textContent = `${displayBalance} $FC`;
...
async function updatePoolAmount() {
    try {
        ...
        if (data && data.total_pool_amount !== undefined) {
            ...
            if (poolElement) {
                poolElement.textContent = `${data.total_pool_amount.toLocaleString()} $FC`;
            }
            ...
            const poolElement = document.getElementById('poolAmount');
            if (poolElement) {
                poolElement.textContent = '100,000 $FC';
            }
        }
    } catch (error) {
            ...
            if (historyContainer) {
                historyContainer.innerHTML = data.rounds.map(round => `
                    <div class="winner-item">
                        ...
                        <div class="winner-amount">${round.total_pool_amount || 0} $FC</div>
                    </div>
                `).join('');
            }
        }
...
function displayLatestResult(round) {
    ...
    const formatAmount = (amount) => {
        return new Intl.NumberFormat('zh-CN').format(amount) + ' $FC';
    };
